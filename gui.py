...
class UserInterface:
    ...
    def _exit_callback(self) -> None:
        ...
        if self.frx is not None:
            ...
            # print("Gracefully disconnect frx first")
            self.i2c_receive.close()
        if self.ftx is not None:
            add_text_to_console("Disconnecting from FTX board...")
            # print("Gracefully disconnect ftx first")
    ...
    def _connect_frx(self, sender=None, data=None) -> None:
        ...
        try:
            ...
            # dpg.add_text("I2cIOError", parent="console_window")
            # print("Could not connect to FRX board, check connection and try again.")
            return
    ...
    def _update_ftx_attn(self) -> None:
        ...
        # dpg.add_text("Setting input attenuation to " + str(new_value) + "...", parent="console_window")
        time.sleep(0.1)
    ...
    def _update_frx_attn(self) -> None:
        ...
        # dpg.add_text("Setting output attenuation to " + str(new_value) + "...", parent="console_window")
        time.sleep(0.1)
        # dpg.add_text("Confirming value was set correctly...", parent="console_window")
        set_value = self.frx.atten.read() / 4
        dpg.set_value("frx_output_attn", set_value)
        # if new_value == set_value:
        #     dpg.add_text("Output attenuation set correctly.", parent="console_window")
        # else:
        if new_value != set_value:
    ...
    def _make_gui(self) -> None:
        ...
        with dpg.window(label="USB-I2C Control Program", tag="primary_window") as main:
            # with dpg.child_window(tag="connection_child_window", width=250, height=85):
            #     with dpg.group(tag="connection_group"):
            #         dpg.add_text("RF Over Fiber Connection")
            with dpg.menu_bar():
            ...
            with dpg.group(label="overall", horizontal=True):
                ...
                with dpg.group(label="right_side"):
                    with dpg.child_window(tag="FRX", width=400, height=440) as frx_win:
